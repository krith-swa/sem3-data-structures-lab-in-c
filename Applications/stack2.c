...
float toPostfix (char exp[]) {
	...
	for (int i=0; exp[i]; i++) {
		if (isOperand(exp[i]))
			...
			char top = getTop(opStack);
			if (top == '(' || top == '[' || top == '{' || !isOperand(exp[i+1]))
				return -4;
			while ( !isEmpty(opStack) && precedence(exp[i])<=precedence(top) )
				push(exprStack,pop(opStack));
			push(opStack,exp[i]);
			}
		}
	...
	while ( !(isEmpty(inputStack)) ) {
		...
		if (isOperand(scan)) {
			//get input for char, convert char to int
			int num;
			printf("Enter value of %c: ",scan);
			scanf("%d",&num);
			if (num>=10 || num<=-10)
				scan = '0'+num;
			else
